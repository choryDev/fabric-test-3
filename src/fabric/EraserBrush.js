  ...
  fabric.util.object.extend(fabric.Canvas.prototype, {
    ...
    _onResize: function () {
      ...
      if (this.backgroundColor) {
        ...
        this.backgroundColor.setPositionByOrigin(
      ...
      if (this.overlayColor) {
        /*
        var clipPath = this.overlayColor.clipPath;
        this.setOverlayColor(this.overlayColor.fill);
        this.overlayColor.clipPath = clipPath;
        */
        /*
         */
        var size = this.overlayColor.getBoundingRect();
        var newSize = { width: this.getWidth(), height: this.getHeight() };
        var delta = {
          x: size.width - newSize.width,
          y: size.height - newSize.height
        };
        var absCenter = new fabric.Point(size.width / 2, size.height / 2);
        var absNewCenter = new fabric.Point(
          newSize.width / 2,
          newSize.height / 2
        );
        var offset = new fabric.Point(delta.x / 2, delta.y / 2);

        var overlayColor = this.overlayColor;
        overlayColor.set(newSize);
        var eraser = this.overlayColor.getEraser();
        if (eraser) {
          var paths = eraser.getObjects("path");
          //eraser._restoreObjectsState();
          eraser.set(newSize);
          eraser._objects[0].set(newSize);
        ...
        //fabric.util.resetObjectTransform(this.overlayColor);
        /*
        fabric.util.applyTransformToObject(
          this.overlayColor,
          fabric.util.multiplyTransformMatrices(
            this.viewportTransform,
            this.overlayColor.calcTransformMatrix()
          )
        );*/
        //var center = this.getCenter();
        /*
        this.overlayColor.setPositionByOrigin(
          new fabric.Point(center.left, 0),
          "center",
          "center"
        );
        */
    ...
    /** @lends fabric.EraserBrush.prototype */ {
      ...
      prepareCanvasOverlayForLayer: function (layer) {
        ...
        var renderOverlayOnTop =
          (image && !this._isErasable(image)) ||
          (color && !this._isErasable(color));
        if (image && this._isErasable(image) === !erasablesOnLayer) {
          this.hideObject(image);
        }
        if (color && this._isErasable(color) === !erasablesOnLayer) {
      ...
      _saveAndTransform: function (ctx) {
        ...
        ctx.globalCompositeOperation = this.inverted
          ? "source-over"
          : "destination-out";
      ...
      _finalizeAndAddPath: function () {
        ...
        canvas.forEachObject(function (obj) {
          if (obj.erasable && path.intersectsWithObject(obj, true, false)) {
